...
class Tidepool():
	...
	def __init__(self):
	    ...
	    xloc = []
            ...
            self.height = 16 #refers to yloc  (> 36 might break it)
	    ...
	    ### KNOWN BUG CAUSED BY MULTIPLE TADPOLES ON 0,0
	    ...
	    while( len(self.list_life) < self.population ):
		    ...
		    # It's possible that spaces dont exist once population is higher in small grids
		    try:
	...
	def tadpoles_go(self):
	    ...
	    for i in range(len(self.list_life)):
			...
			self.list_life[i].clean_mind() #might come back to bite me
